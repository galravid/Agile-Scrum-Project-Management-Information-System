...
$companies = [];
try {
    $stmt = $pdo->query("SELECT c_id, c_data FROM company ORDER BY c_id");
...
<h1>מנהל חברה - דוחות</h1>

<form method="get" action="CompanyReports.php" style="max-width: 400px; margin: 0 auto;">
  <div style="margin-bottom: 30px; text-align: right;">
    <label for="company" style="font-size: 18pt;">בחר חברה לצפייה בדוחות:</label><br>
    <select id="company" name="company_id" required
      style="width: 105%; padding: 12px; font-size: 16pt; border-radius: 10px; border: 1px solid #ccc;">
        <option value="">-- בחר חברה --</option>
        <?php foreach ($companies as $company): ?>
            <?php
                $c_data = json_decode($company['c_data'], true);
  ...
  <div class="button-group">
    <button type="submit">הצג דוחות</button>
  </div>
</form>
<footer>
