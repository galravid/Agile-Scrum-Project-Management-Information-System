...
$companies = [];
try {
    $stmt_companies = $pdo->query("SELECT c_id, c_data FROM company ORDER BY c_id ASC");
    $fetched_companies = $stmt_companies->fetchAll(PDO::FETCH_ASSOC);

    foreach ($fetched_companies as $comp) {
        $decoded_c_data = json_decode($comp['c_data'], true);
        $comp['company_name_from_data'] = $decoded_c_data['name'] ?? 'שם חברה לא ידוע';
        $companies[] = $comp;
    }
} catch (PDOException $e) {
    $message .= "<p style='color: red;'>שגיאה בטעינת רשימת חברות: " . htmlspecialchars($e->getMessage()) . "</p>";
    error_log("Error fetching companies: " . $e->getMessage());
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register_scrum_master'])) {
    ...
    $c_id_raw = trim($_POST['company_name']); 
    $c_id = (int)$c_id_raw; 

    if (empty($name) || empty($login) || empty($password) || empty($email) || empty($phone) || empty($experience_years) || empty($c_id_raw)) {
        $message = "<p style='color: red;'>שגיאה: כל השדות נדרשים.</p>";
    } elseif ($c_id <= 0) { 
        $message = "<p style='color: red;'>שגיאה: יש לבחור חברה חוקית מהרשימה.</p>";
    } else {
        try {
            ...
            if ($stmt_check_login->fetchColumn() > 0) {
                ...
                $stmt_insert_user->execute([$next_uid, $login, $name, (int)$scrumMasterRoleID, $plain_password]);

                if ($stmt_insert_user->rowCount() > 0) {
                    $scrum_master_data_array = [
                        'uid' => $next_uid, // הוספת UID ל-c_data
                        'name' => $name,   // הוספת שם מלא ל-c_data
                        'email' => $email,
                        'phone' => $phone,
                        'experience_years' => $experience_years,
                        'kanban' => [      // הוספת אובייקט kanban ריק
                            'log' => [],
                            'ver' => '2025a',
                            'tasks' => [],
                    ...
                    $scrum_master_data = json_encode($scrum_master_data_array, JSON_UNESCAPED_UNICODE);

                    $stmt_insert_scrum_master = $pdo->prepare("INSERT INTO scrum_team (s_id, c_id, c_data) VALUES (?, ?, ?)");
                    $stmt_insert_scrum_master->execute([$next_uid, $c_id, $scrum_master_data]); // מעבירים את $c_id כפרמטר נפרד

                    if ($stmt_insert_scrum_master->rowCount() > 0) {
                        $message = "<p style='color: green;'>רכז סקראם חדש (UID: " . htmlspecialchars($next_uid) . ") נרשם בהצלחה!</p>";

                        $stmt_new_user = $pdo->prepare("SELECT u.uid, u.login, u.name, u.role, u.password, u.lastused, sm.c_data, sm.c_id as company_id_from_scrum_team FROM user u LEFT JOIN scrum_team sm ON u.uid = sm.s_id WHERE u.uid = ?");
                        $stmt_new_user->execute([$next_uid]);
                        $newlyRegisteredUser = $stmt_new_user->fetch(PDO::FETCH_ASSOC);

                        if ($newlyRegisteredUser && isset($newlyRegisteredUser['c_data'])) {
                            $decoded_scrum_data = json_decode($newlyRegisteredUser['c_data'], true);
                            $newlyRegisteredUser['s_data'] = $decoded_scrum_data;
                            $newlyRegisteredUser['s_data']['c_id'] = $newlyRegisteredUser['company_id_from_scrum_team'];
                        }


                        $pdo->commit();
                        $showUsersTable = true; // אם הרישום הצליח, הצג את הטבלה
                    } else {
                        $message = "<p style='color: red;'>שגיאה: המשתמש נרשם אך לא ניתן לרשום לטבלת רכזי סקראם. אנא נסה שוב.</p>";
                        $pdo->rollBack();
                    }
                } else {
                    $message = "<p style='color: red;'>שגיאה: לא ניתן לרשום את רכז הסקראם. אנא נסה שוב.</p>";
                    $pdo->rollBack();
                }
            }
        } catch (PDOException $e) {
...
.button-group button:hover {
  ...
  padding-top: 70px;
    ...
    /* Consolidated and primary table styling */
    .users-table {
        width: 80%;
        margin: 20px auto;
...
.large-input-field:focus {
    border-color: #007bff; /* צבע מסגרת בעת מיקוד */
    box-shadow: 0 0 0 0.2rem rgba(0, 123, 255, 0.25); /* צל קטן בעת מיקוד */
    outline: none; /* מסיר את קו המתאר ברירת המחדל של הדפדפן */
}

@media (max-width: 500px) {
    .large-input-field {
        font-size: 1em; /* אולי להקטין קצת את הפונט במסכים קטנים יותר */
        padding: 8px; /* להקטין קצת את הריפוד */
    }
}
...
<?php if (!empty($message)): ?>
    ...
    <form method="post">
        ...
        <div style="margin-bottom: 20px;">
            <label for="phone" style="font-size: 18pt;">מספר טלפון:</label><br>
            <input type="text" id="phone" name="phone" required class="large-input-field">
        </div>

        <div style="margin-bottom: 20px;">
            <label for="experience_years" style="font-size: 18pt;">שנות ניסיון:</label><br>
            <input type="text" id="experience_years" name="experience_years" required class="large-input-field">
        </div>
        ...
        <div style="margin-bottom: 30px;">
            <label for="company_name" style="font-size: 18pt;">חברה:</label><br>
            <select id="company_name" name="company_name" required class="large-input-field">
                <option value="" disabled selected>בחר חברה</option>
                <?php foreach ($companies as $company): ?>
                    <option value="<?= htmlspecialchars($company['c_id']) ?>">
                        <?= htmlspecialchars($company['c_id'] . '- ' . $company['company_name_from_data']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="button-group">
            <button type="submit" name="register_scrum_master">הירשם</button>
        </div>
    </form>
            ...
            <table class="users-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td data-label="אימייל"><?= htmlspecialchars($newlyRegisteredUser['s_data']['email'] ?? 'N/A') ?></td>
                        <td data-label="מספר טלפון"><?= htmlspecialchars($newlyRegisteredUser['s_data']['phone'] ?? 'N/A') ?></td>
                        <td data-label="שנות ניסיון"><?= htmlspecialchars($newlyRegisteredUser['s_data']['experience_years'] ?? 'N/A') ?></td>
                        <td data-label="חברה (ID)"><?= htmlspecialchars($newlyRegisteredUser['s_data']['c_id'] ?? 'N/A') ?></td>
    ...
    <?php if ($showUsersTable): ?>
        <h2>טבלת כלל המשתמשים המעודכנת:</h2>
        <div class="users-table-container">
            <table class="users-table">
                <thead>
                    <tr>
