    ...
    if (empty($name) || empty($login) || empty($password)) {
        ...
        try {
            ...
            if ($stmt_check_login->fetchColumn() > 0) {
                ...
                $stmt_insert_user->execute([$next_uid, $login, $name, (int)$RoleID, $plain_password]); // שימוש ב-$plain_password

                if ($stmt_insert_user->rowCount() > 0) {
                    $message = "<p style='color: green;'>בעל מוצר חדש (UID: " . htmlspecialchars($next_uid) . ") נרשם בהצלחה!</p>";
                    ...
                    $message = "<p style='color: red;'>שגיאה: לא ניתן לרשום את בעל המוצר. אנא נסה שוב.</p>";
                    $pdo->rollBack();
                }
            }
